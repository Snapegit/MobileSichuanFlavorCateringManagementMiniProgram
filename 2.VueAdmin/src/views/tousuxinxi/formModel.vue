				...
				<el-row>
					<el-col :span="24">
						<el-form-item label="商家名称" prop="shangjiamingcheng">
							<el-select
								class="list_sel"
								:disabled="!isAdd||disabledForm.shangjiamingcheng?true:false"
								v-model="form.shangjiamingcheng" 
								placeholder="请选择商家名称"
								@change="shangjiamingchengChange">
								<el-option v-for="(item,index) in shangjiamingchengLists" :label="item" :value="item">
								</el-option>
							</el-select>
						</el-form-item>
					</el-col>
...
<script setup>
	...
	const type = ref('')
	//商家名称列表
	const shangjiamingchengLists = ref([])
	//methods
	...
	const getInfo = ()=>{
		context?.$http({
			...
			res.data.data.tousuneirong = res.data.data.tousuneirong?(res.data.data.tousuneirong.replace(reg,'../../../cl35481172/file')):'';
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		context?.$http({
			url: `option/shangjia/shangjiamingcheng`,
			method: 'get'
		}).then(res=>{
			shangjiamingchengLists.value = res.data.data
		})
		//由上级字段带出不可改
		disabledForm.value.shangjiazhanghao = true;
	}
	...
	const shangjiamingchengChange=()=>{
		context?.$http({
			url: `follow/shangjia/shangjiamingcheng?columnValue=` + form.value.shangjiamingcheng,
			method: 'get'
		}).then(res=>{
			if(res.data.data.shangjiazhanghao){
				form.value.shangjiazhanghao = res.data.data.shangjiazhanghao
			}
		})
	}
	//富文本
